...
`doughnut()` draws the classic Doughnut Economics chart, the same boundary primitives at a single instant on polar coordinates. It is the snapshot; `geom_boundary()` is its time-dual.

`conch()` winds the trajectory into a log-spiral. It is an expressive object for communication, covers, and 3D-printed data sculptures, not an analytical instrument. On real data a line chart reads magnitude and timing better and a heatmap reads phase-by-trend better, so the conch ships paired with the Cartesian view rather than in place of it. The stress test that established this lives in the superseded `ggconch/` folder.
...
Pre-CRAN. Core layer, doughnut, and conch build and render. Remaining before submission: `R CMD check` clean in a full R session, roxygen-regenerated man pages, a fuller vignette, and a pkgdown site.
